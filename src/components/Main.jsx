...
const getTabs = () => {
	const originalTabs = TABS.all.items;
	const repeatCount = 64;

	const newTabs = new Array(repeatCount * originalTabs.length);

	for (let i = 0; i < repeatCount; ++i) {
		for (let j = 0; j < originalTabs.length; ++j) {
			newTabs[i * originalTabs.length + j] = originalTabs[j];
		}
	}
